...
<body>
    <header>
        <div class="text-left">
            ...
            <h3 class="text-center">{{ __('Goods Receiving') }}</h3>
            <p class="text-right">Receiving Date :
                {{ $transaction->transaction_at_formatted ?? '[' . __('Receiving Date') . ']' }}</p>
            <p class="text-right">Transaction Category : {{ $transaction->category->name }}</p>
            <h6>Supplier : {{ $transaction->supplier->name ?? '[' . __('Supplier Name') . ']' }}</h6>
        </div>
    </header>
